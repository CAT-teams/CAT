...
def run(model, data_loader, dataset_name, device, output_folder=None, use_cached=False, **kwargs):
...
def inference(model, data_loader, dataset_name, device, output_folder=None, use_cached=False, **kwargs):
    ...
    eval_res = evaluate(dataset=dataset, predictions=predictions, output_dir=output_folder, **kwargs)
    eval_res['metrics']['entropy'] = e.item()
    print('Entropy: {}'.format(eval_res['metrics']['entropy']))
    return eval_res
